...
from ....prompt.repair_template import REPAIR_TEMPLATE_JAVA, REPAIR_TEMPLATE_CPP, REPAIR_TEMPLATE_PYTHON3
...
class LLMCall(Function):
    ...
    def backward(self, response: Variable, prompt: str, system_prompt: str, backward_engine: EngineLM, lang: str = "java"):
        """
        ...
        if response.get_gradient_text() == "":
            self._backward_through_llm_base(children_variables, response, prompt, system_prompt, backward_engine, lang)
        else:
            self._backward_through_llm_chain(children_variables, response, prompt, system_prompt, backward_engine, lang)
    ...
    @staticmethod
    def _construct_llm_base_backward_prompt(backward_info: dict[str, str], lang: str = "java") -> str:
        conversation = CONVERSATION_TEMPLATE.format(**backward_info)
        ...
        if lang == "python3":
            backward_prompt += REPAIR_TEMPLATE_PYTHON3
        elif lang == "cpp":
            backward_prompt += REPAIR_TEMPLATE_CPP
        else:
            backward_prompt += REPAIR_TEMPLATE_JAVA
    ...
    def _backward_through_llm_base(variables: List[Variable], 
        ...
        for variable in variables:
            ...
            backward_prompt = LLMCall._construct_llm_base_backward_prompt(backward_info, lang)
...
class LLMCall_with_in_context_examples(LLMCall):
    ...
    def backward(self, response: Variable, prompt: str, system_prompt: str, in_context_examples: List[str], backward_engine: EngineLM, lang: str = "java"):
        """
        Backward pass through the LLM call. This will register gradients in place.
        ...
        if response.get_gradient_text() == "":
            self._backward_through_llm_base(children_variables, response, prompt, system_prompt, backward_engine, in_context_examples, lang)
        else:
            self._backward_through_llm_chain(children_variables, response, prompt, system_prompt, backward_engine, in_context_examples, lang)
    ...
    def _backward_through_llm_base(variables: List[Variable], 
        ...
        for variable in variables:
            ...
            backward_prompt = LLMCall_with_in_context_examples._construct_llm_base_backward_prompt(backward_info, lang)
