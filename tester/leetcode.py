...
from utils.leetcode_env.environment import LeetCodeEnv
from utils.leetcode_env.types import LeetCodeSubmission, ProgrammingLanguage

def get_element_by_time(list):
    current_second = datetime.now().second
    index = current_second % len(list)
    return list[index][0], list[index][1], index


with open("config.yaml", 'r') as stream:
    try:
        config = yaml.safe_load(stream)
        COOKIE_LIST = config['cookie_config']
...
class LeetcodeValidator(object):

    def __init__(self, cookie_id=0, headers=None):
        self.headers = headers or {
            'User-Agent': 'Mozilla/5.0...',
        }
        ID = cookie_id % len(COOKIE_LIST)
        LEETCODE_SESSION_COOKIE, csrf_token = COOKIE_LIST[ID]['leetcode_session'], COOKIE_LIST[ID]['csrf_token']
        print(f"CURRENT COOKIE ID: {ID}")
        os.environ['LEETCODE_SESSION'] = LEETCODE_SESSION_COOKIE
        self.env = LeetCodeEnv(cooldown=10, csrf_token=csrf_token)
        self.lang_dict = {
    ...
    def test_with_test_msg(self, code: str, task_id: str, language: str) -> tuple[bool, dict]:
        lang = self.lang_dict.get(language)
        sub = LeetCodeSubmission(code=code, lang=lang, question_slug=task_id)
        status, reward, done, submission_result = self.env.step(sub)
        return reward, status, submission_result


if __name__ == '__main__':

    tester = LeetcodeValidator()
